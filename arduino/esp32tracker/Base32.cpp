...
// Had to move myCeil function declaration (defined outside the std namespace) to a location before its usage in the toBase32 function -Ben
...
int Base32::toBase32(byte* in, long length, byte*& out, boolean usePadding)
{
  ...
  size = 8 * myCeil(length / 4.0); // Calculating size of temporary array. Not very precise.
