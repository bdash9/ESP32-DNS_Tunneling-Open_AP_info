...
// ceil functionalready defined in Arduino library so had to change to myCeil -Ben
...
int freeSpacePerQuery(char* domain, int dnsLen)
{
  ...
  return limit - myCeil(float(limit) / float(LABEL_MAX));
}
...
int DNS32::getQueriesLen(char* in, int dnsLen)
{
  ...
  return myCeil(float(len) / float(freeSpace));
}
